...
export function createChecktivClient(deps: ClientDeps = {}): ChecktivClient {
	...
	return {
		async createSession(applicant, opts) {
			const payload: { applicant: ApplicantInput; expectedOutcome?: string } = { applicant };
			if (opts?.expectedOutcome !== undefined) {
				payload.expectedOutcome = opts.expectedOutcome;
			}
